...
def detect_ln(module: DaceModule):
    ...
    applied = module.sdfg.apply_transformations_repeated(DetectLN)
    print(f"Applied {applied} detectlns")
    assert applied > 0
...
torch_tensors_close("input,grad", pt_input.grad, dace_input.grad)

for (name, dace_param), (pt_name,
                         pt_param) in zip(pt_model.named_parameters(),
                                          dace_model.named_parameters()):
    assert 'model.' + name == pt_name
    torch_tensors_close(name, pt_param.grad, dace_param.grad, raise_on_fail=False, summary=True)
...
print(f"input max: {input.max()}, input min: {input.min()}, input mean: {input.mean()}, input std: {input.std()}")
